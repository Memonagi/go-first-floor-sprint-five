...
func (w Walking) Calories() float64 {
	...
	return (CaloriesWeightMultiplier*w.Weight + (math.Pow(speedMInSec, 2)/w.Height)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours
}
...
func (s Swimming) meanSpeed() float64 {
	// вставьте ваш код ниже
	lengthPoolFloat := float64(s.LengthPool)
	countPoolFloat := float64(s.CountPool)
	if s.Duration.Hours() == 0 {
		return 0
	}
	return lengthPoolFloat * countPoolFloat / MInKm / s.Duration.Hours()
}
